...
public class NukkitUserService implements UserService<Player, Player> {
    ...
    public Optional<Player> get(UUID userUUID) {
        return Optional.of(Server.getInstance().getPlayer(userUUID).get());
    }
    ...
    public Optional<UUID> getUUID(String userName) {
        return Optional.of(Server.getInstance().getPlayer(userName).getUniqueId());
    }

    @Override
    public Optional<String> getUserName(UUID userUUID) {
        return Optional.of(Server.getInstance().getPlayer(userUUID).get().getDisplayName());
    }
